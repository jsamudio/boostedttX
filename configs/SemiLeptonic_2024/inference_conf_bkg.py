...
print("--> Crawling active configuration to force 2024 JEC rollback...")

def force_rollback_paths(node):
    if hasattr(node, "keys"):
        # When we hit the lowest level dictionary containing the target JEC params
        if "json_path" in node and "jec_mc" in node:
            # Check if this node is for 2024 by ensuring we don't accidentally overwrite 2022/2023
            if "Summer24" in str(node.get("jec_mc", "")) or "Summer24" in str(node.get("json_path", "")):
                if "fatJet" in str(node["json_path"]):
                    node["json_path"] = "/cvmfs/cms-griddata.cern.ch/cat/metadata/JME/Run3-24CDEReprocessingFGHIPrompt-Summer24-NanoAODv15/2025-12-02/fatJet_jerc.json.gz"
                    node["jec_mc"] = "Summer24Prompt24_V2_MC"
                    node["jec_data"] = "Summer24Prompt24_V2_DATA"
                    node["jer"] = "Summer23BPixPrompt23_RunD_JRV1_MC"
                elif "jet_jerc" in str(node["json_path"]):
                    node["json_path"] = "/cvmfs/cms-griddata.cern.ch/cat/metadata/JME/Run3-24CDEReprocessingFGHIPrompt-Summer24-NanoAODv15/2025-12-02/jet_jerc.json.gz"
                    node["jec_mc"] = "Summer24Prompt24_V2_MC"
                    node["jec_data"] = "Summer24Prompt24_V2_DATA"
                    node["jer"] = "Summer23BPixPrompt23_RunD_JRV1_MC"
        ...
        for key in node.keys():
            force_rollback_paths(node[key])

# Apply directly to the ACTIVE jets_calibration dictionary
if "jets_calibration" in parameters:
    force_rollback_paths(parameters.jets_calibration)

print("--> Active JEC configurations successfully overridden.")
...
cfg = Configurator(
        ...
        datasets = {
            "jsons": [f"{localdir}/datasets/DATA_EGamma.json",
                      ...
                      f"{localdir}/datasets/TTbb_Hadronic_skim.json",
                      f"{localdir}/datasets/TTbb_2L2Nu_skim.json",
                      f"{localdir}/datasets/TTbb_SemiLeptonic_skim.json",
                      f"{localdir}/datasets/TTToHadronic_skim.json",
                      f"{localdir}/datasets/TTTo2L2Nu_skim.json",
                      f"{localdir}/datasets/TTToSemiLeptonic_skim.json"
                ],
            "filter": {
                "samples":  [
                            #"DATA_Muon",
