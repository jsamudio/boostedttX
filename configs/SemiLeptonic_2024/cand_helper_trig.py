...
def ak4_truncate(events):
    ZHCand = events.FatJetSorted[:,0]
    ak4 = events.JetGood
    ZH_ak4_dr = deltaR(ZHCand, ak4)
    ak4_truncated = ak4[ZH_ak4_dr > 0.8]
    events["JetGoodTruncated"] = ak4_truncated
...
def zh_helper(events):
    ...
    events['ZH_M'] = (ZHCand.globalParT3_massCorrX2p * ZHCand.mass * (1-ZHCand.rawFactor))
    events['ZH_sdm'] = ZHCand.msoftdrop
    events['MET_pt'] = met.pt

    # Define 2 leptons and the di-lepton system 
    # Slicing like [:, 0:1] maintains the jagged array shape for smooth vector math later
    lep1 = lep[:, 0:1]
    lep2 = lep[:, 1:2]
    dilep = lep1 + lep2
    ...
    Dynamically padding leptons in case of variable counts
    '''
    lep_pt_pad  = ak.fill_none(ak.pad_none(lep.pt, max(ak.count(lep.pt, axis = 1)), clip=True), np.nan)
    lep_eta_pad = ak.fill_none(ak.pad_none(lep.eta, max(ak.count(lep.eta, axis = 1)), clip=True), np.nan)
    lep_phi_pad = ak.fill_none(ak.pad_none(lep.phi, max(ak.count(lep.phi, axis = 1)), clip=True), np.nan)

    spher, aplan = calc_SandA(
        np.append(ak.to_numpy(ak.fill_none(ak.pad_none(ak4.pt, max(ak.count(ak4.pt, axis = 1)), clip=True), np.nan)), ak.to_numpy(lep_pt_pad), axis=1),
        np.append(ak.to_numpy(ak.fill_none(ak.pad_none(ak4.eta, max(ak.count(ak4.eta, axis = 1)), clip=True), np.nan)), ak.to_numpy(lep_eta_pad), axis=1),
        np.append(ak.to_numpy(ak.fill_none(ak.pad_none(ak4.phi, max(ak.count(ak4.phi, axis = 1)), clip=True), np.nan)), ak.to_numpy(lep_phi_pad), axis=1))
    ...
    '''
    events["ZH_bbvLscore"] = ZHCand.btagBB
    events["outZH_max_ak8pnetMass"] = ak.max(events.FatJetSorted.mass[:,1:], axis=1)
    ...
    # Combinations: ZH and Leptons
    events["ZH_l1_dr"] = ak.flatten(deltaR(ZHCand, lep1))
    events["ZH_l2_dr"] = ak.flatten(deltaR(ZHCand, lep2))
    events["ZH_ll_dr"] = ak.flatten(deltaR(ZHCand, dilep))

    events["ZH_l1_invM"] = ak.flatten((ZHCand + lep1).mass)
    events["ZH_l2_invM"] = ak.flatten((ZHCand + lep2).mass)
    events["ZH_ll_invM"] = ak.flatten((ZHCand + dilep).mass)
    events["dilep_mass"] = ak.flatten(dilep.mass)
    events["dilep_pt"]   = ak.flatten(dilep.pt)
    ...
    # Nearest and second nearest b to LEADING Lepton (lep1)
    b_pt_dRsort_l1, b_eta_dRsort_l1, b_phi_dRsort_l1, b_mass_dRsort_l1 = [make_dRsorted_arr(lep1, bjet, "gt", proc) for proc in [bjet.pt, bjet.eta, bjet.phi, bjet.mass]]
    b_dRsort_l1_vec = zip_4vec(b_pt_dRsort_l1, b_eta_dRsort_l1, b_phi_dRsort_l1, b_mass_dRsort_l1)

    ind_near_b_l1 = ak.argmax(ak.nan_to_num(b_pt_dRsort_l1, nan=-1), axis = 1, keepdims=True)
    near_b_l1 = b_dRsort_l1_vec[ind_near_b_l1]

    ind_subNear_b_l1 = np.where(ind_near_b_l1 + 1 < ak.count(b_pt_dRsort_l1, axis=1), ind_near_b_l1 + 1, ind_near_b_l1)
    subNear_b_l1 = b_dRsort_l1_vec[ind_subNear_b_l1]

    events["l1_nearb_invM"] = ak.flatten((lep1 + near_b_l1).mass)
    events["l1_nearb_dr"] = ak.flatten(deltaR(lep1, near_b_l1))
    events["l1_subNearb_invM"] = ak.flatten((lep1 + subNear_b_l1).mass)
    events["l1_subNearb_dr"] = ak.flatten(deltaR(lep1, subNear_b_l1))

    # Nearest and second nearest b to SUBLEADING Lepton (lep2)
    b_pt_dRsort_l2, b_eta_dRsort_l2, b_phi_dRsort_l2, b_mass_dRsort_l2 = [make_dRsorted_arr(lep2, bjet, "gt", proc) for proc in [bjet.pt, bjet.eta, bjet.phi, bjet.mass]]
    b_dRsort_l2_vec = zip_4vec(b_pt_dRsort_l2, b_eta_dRsort_l2, b_phi_dRsort_l2, b_mass_dRsort_l2)

    ind_near_b_l2 = ak.argmax(ak.nan_to_num(b_pt_dRsort_l2, nan=-1), axis = 1, keepdims=True)
    near_b_l2 = b_dRsort_l2_vec[ind_near_b_l2]

    ind_subNear_b_l2 = np.where(ind_near_b_l2 + 1 < ak.count(b_pt_dRsort_l2, axis=1), ind_near_b_l2 + 1, ind_near_b_l2)
    subNear_b_l2 = b_dRsort_l2_vec[ind_subNear_b_l2]

    events["l2_nearb_invM"] = ak.flatten((lep2 + near_b_l2).mass)
    events["l2_nearb_dr"] = ak.flatten(deltaR(lep2, near_b_l2))
    events["l2_subNearb_invM"] = ak.flatten((lep2 + subNear_b_l2).mass)
    events["l2_subNearb_dr"] = ak.flatten(deltaR(lep2, subNear_b_l2))

    # MT Calculations
    l1_b1_mtb = calc_mtb((lep1+leading_b).pt, (lep1+leading_b).phi, met.pt, met.phi)
    l1_b2_mtb = calc_mtb((lep1+subLeading_b).pt, (lep1+subLeading_b).phi, met.pt, met.phi)
    l2_b1_mtb = calc_mtb((lep2+leading_b).pt, (lep2+leading_b).phi, met.pt, met.phi)
    l2_b2_mtb = calc_mtb((lep2+subLeading_b).pt, (lep2+subLeading_b).phi, met.pt, met.phi)

    events["l1_b1_mtb"] = ak.flatten(l1_b1_mtb)
    events["l1_b2_mtb"] = ak.flatten(l1_b2_mtb)
    events["l2_b1_mtb"] = ak.flatten(l2_b1_mtb)
    events["l2_b2_mtb"] = ak.flatten(l2_b2_mtb)
    ...
    # Corrected overriding bug from original
    events["outZH_b1q_M"] = ak.flatten((leading_b + nearb1_q).mass)
    events["outZH_b2q_M"] = ak.flatten((subLeading_b + nearb2_q).mass)
    ...
    # Uses the dilepton system 
    llbb1qq = (dilep + leading_b + subLeading_b + nearb1_q + subNearb1_q)
    llbb2qq = (dilep + leading_b + subLeading_b + nearb2_q + subNearb2_q)

    events["ZH_llbb1qq_dr"] = ak.flatten(deltaR(ZHCand, llbb1qq))
    events["ZH_llbb2qq_dr"] = ak.flatten(deltaR(ZHCand, llbb2qq))
    ...
    n_ak4_outZhbb = ak.sum(ZH_ak4_dr > 0.8, axis = 1)
    n_ak4_inZhbb = ak.sum(ZH_ak4_dr <= 0.8, axis = 1)   

    events["n_b_inZH"] = n_b_inZhbb
    events["n_b_outZH"] = n_b_outZhbb
    events["n_q_inZH"] = n_q_inZhbb
    events["n_q_outZH"] = n_q_outZhbb
    events["n_ak4_inZH"] = n_ak4_inZhbb
    events["n_ak4_outZH"] = n_ak4_outZhbb
    ...
    # Changed lep.pt to the summed dilepton PT 
    ht_b = ak.sum(ak.nan_to_num(b_pt_ptsort, nan=0), axis=1)
    sc_pt_outZH = ht_b + ak.sum(ak.nan_to_num(q_pt_ptsort, nan=0), axis=1) + ak.sum(ak.nan_to_num(lep.pt, nan=0), axis=1)
    ...
    events["n_ak8_ZHbb"] = ak.sum(ak.where(ak8.particleNet_XbbVsQCD > 0.8, 1, 0), axis=1)