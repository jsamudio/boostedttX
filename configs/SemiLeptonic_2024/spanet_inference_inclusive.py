...
from custom_scale_factors import sf_scaleweights, sf_btag_wp, sf_ele_trig
...
class ZHbbSpanetProcessor (BaseProcessorABC):
    ...
    def process_extra_after_presel(self, variation):
        ...
        if 'DATA' not in self._sample:
            ...
            self.events['btag_sfup'] = btag_sf['up'][0]
            self.events['btag_sfdown'] = btag_sf['down'][0]
            ...
            print('puweight', self.events['puWeight'])
            print('puweight up', self.events['puWeight_up'])
        ...
        if (("TTbb" in self._sample) | ("TTTo" in self._sample) | ("ttbb" in self._sample)):
