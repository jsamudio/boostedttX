...
class ZHbbBaseProcessor (BaseProcessorABC):
    ...
    def apply_object_preselection(self, variation):
        ...
        self.events['JetGood'], self.jetGoodMask = jet_selection(self.events, "Jet", self.params, self._year, "LeptonGood", self.params.object_preselection.btag.tagger) #use PC default
        self.events['FatJetGood'], self.fatJetGoodMask = jet_selection(self.events, "FatJet", self.params, self._year, "LeptonGood", "PNetMD") # they use some MSD cut which we don't want
    ...
    def do_parton_matching(self) -> ak.Array:
        ...
        if self._sample in ['ttHTobb', 'TTZToQQ']:
        ...
        elif self._sample in ['TTZToBB', 'TTZToQQ']:
            prov = get_partons_provenance_ttHbb(
                ak.Array(quarks.pdgId, behavior={}), ak.ArrayBuilder()
            ).snapshot()
            self.events["HiggsParton"] = self.events.LHEPart[
    ...
    def define_common_variables_after_presel(self, variation):
        ...
        for wp, val in self.params.btagging.working_point[self._year]["btagging_WP"].items():
            self.events["JetGood"] = ak.with_field(
                self.events.JetGood,
                ak.values_astype(self.events.JetGood.btagB > val, int),
                f"btag_{wp}"
            )
    ...
    def process_extra_after_presel(self, variation):
        ...
        #if 'TTbb' in self._sample: # probably can just use the xsec
