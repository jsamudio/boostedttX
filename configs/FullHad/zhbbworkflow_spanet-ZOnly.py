...
class ZHbbBaseProcessor (BaseProcessorABC):
    ...
    def do_parton_matching(self) -> ak.Array:
        ...
        # Select b-quarks at Gen level, coming from H->bb decay
            ...
            zboson = self.events.GenPart[
                (self.events.GenPart.pdgId == 23)
                & (self.events.GenPart.hasFlags(['fromHardProcess']))
            ]
            zboson = zboson[ak.num(zboson.childrenIdxG, axis=2) == 2]
            self.events["ZGen"] = zboson

            matched_zboson, matched_fatjets, deltaR_matchedAK8 = object_matching(
                zboson, self.events.FatJetSorted, dr_min=self.dr_min
            )
            print("Matched fatjets:", matched_fatjets)
            
            z_partons = ak.with_field(
                ak.flatten(zboson.children, axis=2), 25, "from_part"
            )
            # DO NOT sort b-quarks by pt
            # if not we are not able to match them with the provenance
            quarks = ak.with_name(
                ak.concatenate((quarks, zboson_partons), axis=1),
                name='PtEtaPhiMCandidate',
            )
        else:
            higgs = self.events.GenPart[
