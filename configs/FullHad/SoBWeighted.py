...
class DNN_datasets:
    ...
    #bkg = ['TTBar', 'ttbb']
    dnn_vars = []
    cut_vars = ['process','ZH_pt','MET_pt','ZH_M', 'ZH_bbvLscore', 'norm_weight', 'genWeight', 'topptWeight', 'nAK4inAK8', 'n_ak4jets']
    output_dir = './nn_files'

    def __init__(self):
        self.s_df, self.b_df = self.get_sigbkg()
        self.sb_df = pd.concat([self.s_df,self.b_df])
        #self.sb_df = self.s_df
        self.plot_dnnHist()
    ...
    def plot_dnnHist(self):
        ...
        sumS = []
        sumB = []
        maximum = max(self.sb_df['nAK4inAK8'])
        binEdges = np.arange(0, maximum+1, 0.1 if maximum < 1.5 else 1 if maximum <= 20 else 5 if maximum <= 100 else 20)

        for i in bkgs+sigs:
            norm_weight = ((np.asarray(self.sb_df[(self.sb_df['process'] == i)]['norm_weight'].to_numpy(), dtype = float)))
            norm_weight2 = (np.asarray(self.sb_df[(self.sb_df[genmatchreq] == True) & (self.sb_df['process'] == i)]['norm_weight'].to_numpy(), dtype = float))
            weight = np.asarray(self.sb_df[(self.sb_df['process'] == i)]['genWeight'].to_numpy(), dtype = float)
            weight2 = np.asarray(self.sb_df[(self.sb_df[genmatchreq] == True) & (self.sb_df['process'] == i)]['genWeight'].to_numpy(), dtype = float)
            topptWeight = np.asarray(self.sb_df[(self.sb_df['process'] == i)]['topptWeight'].to_numpy(), dtype = float)
            topptWeight2 = np.asarray(self.sb_df[(self.sb_df[genmatchreq] == True) & (self.sb_df['process'] == i)]['topptWeight'].to_numpy(), dtype = float)

            norm_weight = (topptWeight * norm_weight * np.sign(weight))
            norm_weight2 = (topptWeight2 * norm_weight2 * np.sign(weight2))

            if i in bkgs:
                n, bins, patches = ax.hist(self.sb_df['nAK4inAK8'][(self.sb_df['process'] == i)], bins=binEdges, stacked=False,
                    histtype='step', label=f'{i}', weights=norm_weight/(sum(norm_weight)))
                print("okay")
            else:
                #n, bins, patches = ax.hist(self.sb_df['nAK4inAK8'][(self.sb_df['process'] == i)], bins= np.arange(0,10,1), stacked=True,
                #    histtype='step', label=f'{i}', weights=norm_weight, density=True)
                n, bins, patches = ax.hist(self.sb_df['nAK4inAK8'][(self.sb_df['process'] == i)], bins=binEdges, stacked=False,
                    histtype='step', label=f'{i}', weights=norm_weight/(sum(norm_weight)))
                #n, bins, patches = ax.hist(self.sb_df['nAK4inAK8'][(self.sb_df[genmatchreq] == True) & (self.sb_df['process'] == i)], bins= np.arange(0,10,1), stacked=True,
                #    histtype='step', label=f'{i} GenMatched', weights=norm_weight2, density=True)
                n, bins, patches = ax.hist(self.sb_df['nAK4inAK8'][(self.sb_df[genmatchreq] == True) & (self.sb_df['process'] == i)], bins=binEdges, stacked=False,
                    histtype='step', label=f'{i} GenMatched', weights=norm_weight2/(sum(norm_weight2)))
                print(n)
        ...
        print("S:", np.sum(sumS), "B:", np.sum(sumB))
        ...
        ax.set_ylabel("Fraction of yield")
        ax.set_xlabel(r"nAK4inAK8")
