...
class DNN_datasets:
    ...
    def get_sigbkg(self):
            ...
            for j in filein['columns'][f'{i}'].keys():
                for var in pre_vars+[genmatchreq]:
                    if (var == 'norm_weight'):
                        ...
                        inner_list = filein['columns'][f'{i}'][f'{j}']['btag_mask'][f'spanet_output_{var}'].value.tolist()
                    else:
                        inner_list = filein['columns'][f'{i}'][f'{j}']['btag_mask'][f'events_{var}'].value.tolist()
        ...
        #s_df = s_df[s_df[genmatchreq] == False]
    ...
    def plot_dnnHist(self):
        ...
        for i in sig + bkg:
            ...
            if i in bkg:
                n, bins, patches = ax.hist(self.sb_df['newgenm_NN'][(self.sb_df[genmatchreq] == False) & cuts & plotcut & (self.sb_df['process'] == i)], bins=self.nn_bins, stacked=False,
                    histtype='step', range= (0,1), label=label_dict[i], weights=norm_weight, density=True)
            else:
                n, bins, patches = ax.hist(self.sb_df['newgenm_NN'][(self.sb_df[genmatchreq] == True) & cuts & plotcut & (self.sb_df['process'] == i)], bins=self.nn_bins, stacked=False,
            #n, bins, patches = ax.hist(self.sb_df['newgenm_NN'][cuts & plotcut & (self.sb_df['process'] == i)], bins=10, stacked=False,
                        histtype='step', range= (0,1), label=label_dict[i] + "$_{matched}$", weights=norm_weight2, density=True)
                n, bins, patches = ax.hist(self.sb_df['newgenm_NN'][(self.sb_df[genmatchreq] == False) & cuts & plotcut & (self.sb_df['process'] == i)], bins=self.nn_bins, stacked=False,
            #n, bins, patches = ax.hist(self.sb_df['newgenm_NN'][cuts & plotcut & (self.sb_df['process'] == i)], bins=10, stacked=False,
                        histtype='step', range= (0,1), label=label_dict[i], weights=norm_weight, density=True)
        ...
        ax.set_xlabel(r'NN Score', fontsize=10)
        #ax2.grid(True)
        ...
        plt.savefig("pdf/sl_dnn.pdf", bbox_inches='tight')

    def get_NN_bins(self):
        cuts = dnn_cut(self.sb_df)
        genm = (self.sb_df[genmatchreq] == True)
        # currently not binning by pt...
        nn_df = np.asarray(self.sb_df[ cuts & genm & (self.sb_df['newgenm_NN'] <= 1) & (self.sb_df['newgenm_NN'] > 0.) & (self.sb_df['ZH_pt'] > 200)]['newgenm_NN'].to_numpy(), dtype = float)
        norm_weight = np.asarray(self.sb_df[cuts & genm & (self.sb_df['newgenm_NN'] <= 1) & (self.sb_df['newgenm_NN'] > 0.) & (self.sb_df['ZH_pt'] > 200)]['norm_weight'].to_numpy(), dtype = float)
        weight = np.asarray(self.sb_df[cuts & genm & (self.sb_df['newgenm_NN'] <= 1) & (self.sb_df['newgenm_NN'] > 0.) & (self.sb_df['ZH_pt'] > 200)]['genWeight'].to_numpy(), dtype = float)
        topptWeight = np.asarray(self.sb_df[cuts & genm & (self.sb_df['newgenm_NN'] <= 1) & (self.sb_df['newgenm_NN'] > 0.) & (self.sb_df['ZH_pt'] > 200)]['topptWeight'].to_numpy(), dtype = float)
        norm_weight = (topptWeight * norm_weight * np.sign(weight))
