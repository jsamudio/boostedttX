...
def match_gen_lep(events, lepton_collection, label=''):
    ...
    lep_match_dr = deltaR(lepton_collection, events.GenPart[islep])
    events["matchedGenLep"+label] = (ak.sum(lep_match_dr <= 0.1, axis=1) > 0)
...
def match_gen_sig(events, sample):
    ...
    events['matchedGen_Zbb']  = ((ak.sum(zh_match, axis=1) > 0) & (ak.sum(isZbb,axis=1) >  0))
    events['matchedGen_Hbb']  = ((ak.sum(zh_match, axis=1) > 0) & (ak.sum(isHbb,axis=1) >  0))
    ...
    events['matchedGen_ZHbb'] = ((ak.sum(zh_match, axis=1) > 0) & ((ak.sum(isZbb,axis=1) + ak.sum(isHbb, axis=1)) > 0))
    events['matchedGen_Zqq']  = ((ak.sum(zh_match, axis=1) > 0) & (ak.sum(isZqq,axis=1) >  0))
    #
