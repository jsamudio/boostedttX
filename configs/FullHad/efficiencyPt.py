...
class DNN_datasets:
    ...
    #bkg = ['TTBar', 'ttbb']
    dnn_vars = NN_vars
    cut_vars = ['process','ZH_pt','MET_pt','ZH_M', 'ZH_bbvLscore', 'norm_weight', 'genWeight', 'topptWeight', 'ttzbb', 'ttbb', 'ttcc', 'ttlf', 'tthbb']
    output_dir = './nn_files'
        ...
        for i in sig:
            ...
            for j in filein['columns'][f'{i}'].keys():
                for var in pre_vars+[genmatchreq]:
                    ...
                    elif (var in ['tthbb', 'ttbb', 'ttlf', 'ttcc']):
    ...
    def get_denominator(self):
        ...
        for i in sigs:
            ...
            n = np.sum(norm_weight)
            print(i, "Number of total events in bin: ", n)
            if i in sigs:
                sumS.append(n)
        return sumS
    ...
    def get_numerator(self):
        ...
        sumB = []
        cuts = (
            (self.sb_df['n_ak4jets']   >= 5)       &
            (self.sb_df['n_b_outZH'] == 2) &
            (self.sb_df['ZH_pt']       >= ptlow)     &
            (self.sb_df['ZH_pt']       <  pthigh) &
            #(self.sb_df['ZH_M']       >= 50)     &
            #(self.sb_df['ZH_M']       <  200) &
            (self.sb_df['sigVsbkg'] > 0.951)
        )
        print(sum(cuts))
        for i in sigs:
            norm_weight = ((np.asarray(self.sb_df[cuts & (self.sb_df['process'] == i)]['norm_weight'].to_numpy(), dtype = float))/(41.529))*137.596
    ...
    def plot_dnnHist(self):
        ...
        for i in bkgs+sigs:
            ...
            norm_weight = (topptWeight * norm_weight * np.sign(weight))
            norm_weight2 = (topptWeight2 * norm_weight2 * np.sign(weight2))
            print("Summed weight:", np.sum(norm_weight))
        ...
        fig.text(tex_x_corr,0.53, r'SPANet score > 0.908', fontsize=10)
        ...
        #if ptrange == 'l':
        #    plt.savefig(f"pdf/sl_mass_{ptrange}_{dirname}_2bin.pdf", bbox_inches='tight')
        #if ptrange == 'm':
        #    plt.savefig(f"pdf/sl_mass_{ptrange}_{dirname}_2bin.pdf", bbox_inches='tight')
        #if ptrange == 'h':
        #    plt.savefig(f"pdf/sl_mass_{ptrange}_{dirname}_2bin.pdf", bbox_inches='tight')
