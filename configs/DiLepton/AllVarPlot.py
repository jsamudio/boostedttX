...
class DNN_datasets:
    ...
    #bkg = ['TTBar', 'ttbb']
    dnn_vars = NN_vars
    cut_vars = ['process','ZH_pt','MET_pt','ZH_M', 'ZH_bbvLscore', 'norm_weight', 'genWeight', 'topptWeight', 'n_ak4jets']
    output_dir = './nn_files'
    ...
    def plot_dnnHist(self):
        ...
        for k in self.dnn_vars:
            ...
            binning = np.arange(0, maximum+1 if maximum < 1000 else 1000, 0.1 if maximum < 8 else 1 if maximum <= 20 else 5 if maximum <= 100 else 20)
